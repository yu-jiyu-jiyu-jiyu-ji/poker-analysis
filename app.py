...
def pick_card(label: str, key: str, choices: list[str]) -> str:
    ...
    value = st.selectbox(label, choices, index=idx, key=key, label_visibility="collapsed" if not label else "visible")
    return value
...
add_cols = st.columns([1.2,1.6,.8])
with add_cols[0]:
    pos_options = remaining_positions()
    vp = st.selectbox("ポジション", pos_options if pos_options else ["（空きなし）"], key="vill_add_pos")
with add_cols[1]:
    vt = st.selectbox("タイプ", ["tight-passive","tight-aggressive","loose-passive","loose-aggressive"], key="vill_add_type")
with add_cols[2]:
    st.button("＋ 追加", use_container_width=True, disabled=not pos_options, key="vill_add_btn")
    if pos_options and st.session_state.get("vill_add_btn"):
        st.session_state.villains.append({"pos": vp, "type": vt})
        st.session_state["vill_add_btn"] = False
for i, v in enumerate(list(st.session_state.villains)):
    c = st.columns([2,3,1])
    c[0].markdown(f"- **{v['pos']}**")
    c[1].markdown(f"タイプ：{v['type']}")
    if c[2].button("削除", key=f"vill_del_{i}"):
    ...
    for i, r in enumerate(rows):
        ...
        with cols[1]:
            ...
            rows[i]["act"] = st.selectbox("アクション", acts,
                                          ...
                                          key=f"{key}_act_{i}")
        with cols[2]:
            rows[i]["amt"] = st.text_input("金額（数字のみ）", r.get("amt",""), key=f"{key}_amt_{i}", placeholder="200 / 5000")
        with cols[3]:
            if st.button("🗑️", key=f"{key}_del_{i}"): to_del.append(i)
    for i in reversed(to_del): rows.pop(i)
    if st.button(f"+ 行を追加（{title}）", key=f"{key}_add"):
        rows.append({"pos":"Hero" if postflop else "UTG", "act":"check" if postflop else "open", "amt":""})
    parts = []
    for r in rows:
        ok, text, err = normalize_line(r["pos"], r["act"], r["amt"])
        parts.append(text if ok else "❌")
